...
portal expose 127.0.0.1:8080 --udp --udp-addr 127.0.0.1:19132 --name game --identity-path /absolute/path/to/identity.json
```
...
`run`, `restart`, and `stop` are service-wide. They apply to every `[[tunnels]]` entry owned by that `service_name`. Do not point a second config at the default `portal-agent` service if an existing agent already has unrelated tunnels.
