...
ECH-enabled tunnels retain plaintext-SNI fallback routing. This lets clients that do not obtain or use the ECHConfigList connect through the public hostname without weakening tenant TLS passthrough. Optional IVNP forwarding carries the encrypted stream to the authorized backhaul relay. The tunnel endpoint must still be configured for the ingress hostname and its ECH material.
