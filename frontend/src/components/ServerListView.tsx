...
function normalizeKnownRelays(
  ...
  relays?.forEach((relay) => {
    ...
    knownRelays.push({
      ...
      compatible: true,
      protocolVersion: relay.version?.trim() || undefined,
    });
  });
...
export function buildKnownRelays(
  discovery: DiscoveryResponse | undefined,
  currentRelayURL: string
): KnownRelay[] {
  return mergeIncompatibleRelays(
...
function relayBadgeLabel(relay: KnownRelay): string {
  const version = relay.protocolVersion
    ? `discovery ${relay.protocolVersion}`
    : "discovery";
  return relay.compatible ? version : `${version} · incompatible`;
}
...
export function ServerListView({
  ...
  const serverItems = filteredServers;
  const favoriteIds = useMemo(() => new Set(favorites), [favorites]);
  const currentRelayURL = useMemo(() => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        nextKnownRelays = buildKnownRelays(discovery, currentRelayURL);
      } catch {
  ...
  const serverCards = serverItems.map((server) => (
    <ServerCard
      key={server.id}
      server={server}
  ...
  const gridClasses =
    "grid grid-cols-1 gap-6 py-4 min-[500px]:py-6 min-[500px]:grid-cols-2 md:grid-cols-3";
  const serverGrid =
          ...
          <main className="z-0 flex-1 pb-14">
            ...
            >
              <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                <TunnelCommandModal leases={leases} />
              </div>
                      ...
                      >
                        ...
                        <div className="flex shrink-0 items-center gap-2">
                          <span
                            className={
                              relay.compatible
                                ? "rounded-sm bg-secondary/70 px-2.5 py-1 font-mono text-[11px] font-medium text-text-muted ring-1 ring-border"
                                : "rounded-sm bg-red-500/10 px-2.5 py-1 font-mono text-[11px] font-medium text-red-500 ring-1 ring-red-500/20"
                            }
                          >
                            {relayBadgeLabel(relay)}
                          </span>
                        </div>
                      </div>
                    ))}
