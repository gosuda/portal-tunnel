...
func NewRoutePolicy() *RoutePolicy {
	...
	return &RoutePolicy{
		...
		reverse: reverse,
		variant: variant,
		nodes:   make(map[uint32]RelayHealth),
	}
}

// UpdateNodeHealth registers or updates the health metrics for a relay node.
func (p *RoutePolicy) UpdateNodeHealth(nodeID uint32, h RelayHealth) {
	p.mu.Lock()
	defer p.mu.Unlock()
	p.nodes[nodeID] = h
}
...
func (p *RoutePolicy) BuildRouteWithLoad(ingress uint32, candidates []uint32, maxHops int, load NodeLoad, congestionLatencyMs float64) ([]uint32, error) {
...
func ensureMagicSquare(grid []uint16) {
	if !isMagicSquare(grid) {
		panic("policy: orthogonal latin grid must remain a magic square")
	}
}
