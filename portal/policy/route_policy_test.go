...
func TestBuildRouteSwitchesOnCongestion(t *testing.T) {
	...
	normal, err := p.BuildRouteWithLoad(42, candidates, 3, NodeLoad{AvgLatencyMs: 20}, 100)
	if err != nil {
		t.Fatalf("normal route error: %v", err)
	}
	congested, err := p.BuildRouteWithLoad(42, candidates, 3, NodeLoad{AvgLatencyMs: 180}, 100)
	if err != nil {
...
func TestUpdateNodeHealthAndActiveCount(t *testing.T) {
	p := NewRoutePolicy()
	p.UpdateNodeHealth(10, RelayHealth{Healthy: true, PingLatencyMs: 5})
	p.UpdateNodeHealth(20, RelayHealth{Healthy: true, PingLatencyMs: 8})
	p.UpdateNodeHealth(30, RelayHealth{Healthy: false, PingLatencyMs: 200})
...
func TestMarkSlowFallbackEnforcesMinimum(t *testing.T) {
	...
	p.UpdateNodeHealth(1, RelayHealth{Healthy: true, PingLatencyMs: 10})
	p.UpdateNodeHealth(2, RelayHealth{Healthy: true, PingLatencyMs: 10})
	...
	p.UpdateNodeHealth(3, RelayHealth{Healthy: true, PingLatencyMs: 10})
	if !p.MarkSlowFallback(1) {
...
func TestMarkSlowFallbackIdempotent(t *testing.T) {
	p := NewRoutePolicy()
	p.UpdateNodeHealth(1, RelayHealth{Healthy: true, PingLatencyMs: 5})
	p.UpdateNodeHealth(2, RelayHealth{Healthy: true, PingLatencyMs: 5})
	p.UpdateNodeHealth(3, RelayHealth{Healthy: true, PingLatencyMs: 5})
...
func TestFallbackNodesAreScoredLast(t *testing.T) {
	...
	p.UpdateNodeHealth(200, RelayHealth{Healthy: true, PingLatencyMs: 1, Fallback: true})
	for _, id := range []uint32{101, 102, 103, 104, 105} {
		p.UpdateNodeHealth(id, RelayHealth{Healthy: true, PingLatencyMs: 10})
	}
...
func TestNonLinearLoadUsesVariantGrid(t *testing.T) {
	...
	p.UpdateNodeHealth(100, RelayHealth{Healthy: true, PingLatencyMs: 1})
	p.UpdateNodeHealth(200, RelayHealth{Healthy: true, PingLatencyMs: 1000})
	...
	for _, id := range candidates {
		p2.UpdateNodeHealth(id, RelayHealth{Healthy: true, PingLatencyMs: 10})
	}
